...
| `probe_sepa_rainfall.py` | SEPA KiWIS time-series API (probed 2026-07-05) | Keyless, OGL (attribute SEPA). ~380 Scottish rain gauges, 15-min mm totals, live within the hour, archives to the 1990s. **Rain-amounts ground truth for Scotland**; all 6 Scottish stations have a live gauge ≤15 km (Edinburgh's is 0.2 km). Many gauges per `getTimeseriesValues` call. Old `apps.sepa.org.uk` API is dead. |
| `probe_nrw_rainfall.py` | NRW rivers-and-seas API (needs `NRW_API_KEY` in `.env`; probed 2026-07-05) | Open data, free subscription key from api-portal.naturalresources.wales. ~150 Welsh rain gauges, 15-min mm totals, live within ~30 min. **Rain-amounts ground truth for Wales** (Bangor 3.2 km, Cardiff 4.5 km — vs Cardiff's old EA gauge 22.6 km away in Somerset). Gotchas: `statusEN` lies (use `latestTime`), per-station Rainfall parameter IDs, unwindowed historical = a full year (window with `from`/`to`). |
